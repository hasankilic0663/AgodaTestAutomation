...
public abstract class BaseMasterPage {
    ...
    private static  BeinTextBox INPUT_Konum =
            new BeinTextBox(PageElementModel.selectorNames.XPATH,
                    "//input[@id='address' and contains(@class,'address-input')]");
    private static BeinButton BTN_KonumOnayla =
            new BeinButton(PageElementModel.selectorNames.XPATH , "//input[contains(@class,'address-input') and @placeholder='Lütfen adresinizi girin']");

    public void clickKonum(String konumAdi) {
        log.info("ENTERING clickKonum");
        INPUT_Konum.waitUntilVisible();
        INPUT_Konum.clearText();
        INPUT_Konum.type(konumAdi);
        BTN_KonumOnayla.click();
        log.info("Konum girildi ve onaylandı: " + konumAdi);
        log.info("Konum başarıyla girildi: " + konumAdi);
    }
    public void logout(){
